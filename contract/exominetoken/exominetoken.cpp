...
void token::mine( const name& miner ) {
    ...
    const time_point_sec now = time_point_sec().sec_since_epoch();

    if(last != _last.end()) check(now >= last->last_mine + time_point_sec(60), "Please wait at least 1 minute before mine again"); 
