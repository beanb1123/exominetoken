...
void token::mine( const name& miner ) {
    ...
    if(last != _last.end()) check(eosio::time_point::sec_since_epoch() >= last->last_mine + 60, "Please wait at least 1 minute before mine again"); 
...
//    st = statstable.get( sym.raw(), "no balance with specified symbol" );
    ...
    if(last == _last.end()) {        
      _last.emplace( get_self(), [&]( auto& s ) {
        s.miner = miner;
        s.last_mine = eosio::time_point::sec_since_epoch();
      });
    } else {
